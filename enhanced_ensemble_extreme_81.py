...
CONFIG = {
    ...
    # 🔥 极致优化
    'batch_size': 80,          # ⬇️ 略微减小batch提高泛化
    'lr': 0.0002,              # ⬇️ 更小学习率
    'weight_decay': 0.0005,    # ⬆️ 更强正则化
    'epochs': 100,             # ⬆️ 更多训练轮数
    'warmup_epochs': 10,       # ⬆️ 更长预热
    
    # 🔥 最强数据增强
    'aug_strength': 'very_strong',  # ⬆️ 使用最强增强
    'mixup_alpha': 0.3,        # ⬆️ 更强Mixup
    'cutmix_alpha': 1.0,
    'cutmix_prob': 0.6,        # ⬆️ 更高CutMix概率
    
    # 模型配置
    'use_ema': True,
    'ema_decay': 0.9995,
    'label_smoothing': 0.1,
    'dropout': 0.6,            # ⬆️ 更强Dropout
    
    # 🔥 所有技术全开
    'use_sam': True,           # ✅ 开启SAM（虽然慢）
    'gradient_clip': 1.0,
    'early_stop_patience': 20, # ⬆️ 更有耐心
    'use_swa': True,
    'swa_start': 70,           # SWA从70%处开始
    
    # 🔥 测试时增强
    'use_tta': True,
    'tta_transforms': 7,       # ⬆️ 更多TTA变换
    
    # 🔥⭐ 最多模型集成
    'models': [
        'resnet50',
        'resnet101',
        'resnet152',           # ⬆️ 新增最深ResNet
        'efficientnet_b4',
        'efficientnet_b5',
        'convnext_small',
        'convnext_base'        # ⬆️ 新增更大ConvNeXt
    ],
    ...
    'num_workers': 16,
    'seed': 42,
    'class_map': {'buildings': 0, 'forest': 1, 'glacier': 2, 
...
def train_single_model(model_name, train_loader, val_loader, fold=0):
    ...
    # ===== 断点续训：检查是否存在checkpoint =====
    checkpoint_path = f"{CONFIG['model_save_dir']}/checkpoint_{model_name}_fold{fold}.pth"
    start_epoch = 0
    best_val_acc = 0.0
    patience_counter = 0
    
    if os.path.exists(checkpoint_path):
        print(f"\n🔄 发现checkpoint，从断点继续训练...")
        try:
            # PyTorch 2.6需要显式设置weights_only=False
    ...
    for epoch in range(start_epoch, CONFIG['epochs']):
        ...
        for images, labels in train_bar:
            ...
            if CONFIG['use_sam']:
                # SAM优化器需要两次前向传播
                # 注意: SAM与混合精度训练不兼容，这里禁用混合精度
                enable_amp = False  # 强制禁用，避免与SAM冲突
                ...
                def closure():
                    nonlocal loss_value, outputs_value
                    optimizer.zero_grad()
                    with torch.cuda.amp.autocast(enabled=enable_amp):
                        outputs_value = model(images)
                        if mixed:
                            loss_value = lam * criterion(outputs_value, labels_a) + (1 - lam) * criterion(outputs_value, labels_b)
                        else:
                            loss_value = criterion(outputs_value, labels)
                    loss_value.backward()
                    return loss_value
        ...
        # ===== 保存checkpoint（每个epoch都保存，用于断点续训）=====
        checkpoint_dict = {
            'epoch': epoch,
        ...
        torch.save(checkpoint_dict, f"{CONFIG['model_save_dir']}/checkpoint_{model_name}_fold{fold}.pth")
...
def train():
    ...
    for model_name in CONFIG['models']:
        ...
        if CONFIG['use_swa'] and os.path.exists(swa_path):
            checkpoint = torch.load(swa_path, weights_only=False)
            print(f"加载SWA模型: {model_name}")
        else:
            checkpoint = torch.load(normal_path, weights_only=False)
